...
Research repository for the vivarium_spatial project.
...
all necessary requirements as follows::
  ...
  :~$ conda create --name=vivarium_spatial python=3.11 git git-lfs
  ...conda will download python and base dependencies...
  :~$ conda activate vivarium_spatial
  (vivarium_spatial) :~$ git clone https://github.com/ihmeuw/vivarium_spatial.git
  ...git will copy the repository from github and place it in your current directory...
  (vivarium_spatial) :~$ cd vivarium_spatial
  (vivarium_spatial) :~$ pip install -e .
...
`pip install -e .` with the following::
  ...
  (vivarium_spatial) :~$ pip install -r requirements.txt
...
install hdf tooling from within your environment like so::
  ...
  (vivarium_spatial) :~$ conda install hdf5

The ``(vivarium_spatial)`` that precedes your shell prompt will probably show
...
``src/vivarium_spatial`` package directory:
...
- ``components``
  ...
  the vivarium_spatial project. You should work with the
...
With this model specification file and your conda environment active, you can then run simulations by, e.g.::
   ...
   (vivarium_spatial) :~$ simulate run -v /<REPO_INSTALLATION_DIRECTORY>/vivarium_spatial/src/vivarium_spatial/model_specifications/model_spec.yaml
