...
class ParticleVisualizer(Component):
    ...
    def __init__(
        ...
        self.collision_history = {"low": [], "medium": [], "high": []}
    ...
    def _setup_unit_square_view(self) -> None:
        ...
        self.simulation_bounds = pygame.Rect(x_offset, y_offset, self.scale, self.scale)
    ...
    def get_color_for_whimsy(self, whimsy: float) -> Tuple[int, int, int]:
        ...
        return tuple(
            int(
                self.low_whimsy_color[i]
                + (self.high_whimsy_color[i] - self.low_whimsy_color[i]) * whimsy
            )
            for i in range(3)
        )
                ...
                for i in range(len(positions) - 1):
                    ...
                    if dx < 0.5 and dy < 0.5:
                        ...
                        pygame.draw.line(
                            ...
                            point1,
                            point2,
                            width=self.trail_size,
                        )
                        pygame.draw.aaline(
                            self._trail_surface, particle_color, point1, point2
                        )
    ...
    def _draw_trend_chart(self) -> None:
        ...
        max_collisions = max(
            max(
                max(history) if history else 0 for history in self.collision_history.values()
            ),
        ...
        pygame.draw.line(
            ...
            (
                self.chart_size[0] - self.chart_padding,
                self.chart_size[1] - self.chart_padding,
            ),
        )
        pygame.draw.line(
            self._chart_surface,
        ...
        for category, history in self.collision_history.items():
            ...
            if len(points) > 1:
                pygame.draw.lines(
                    self._chart_surface, self.chart_colors[category], False, points, 2
                )
        ...
        for category, color in self.chart_colors.items():
            pygame.draw.line(
                ...
                (self.chart_size[0] - 60, legend_y),
                2,
            )
            font = pygame.font.Font(None, 20)
            text = font.render(
                f"{category}: {self.collision_history[category][-1]}", True, color
            )
        ...
        self._chart_surface.blit(
            zero_label, (5, self.chart_size[1] - self.chart_padding - 10)
        )
    ...
    def _draw_border(self) -> None:
        ...
        pygame.draw.rect(
            self._screen, self.border_color, self.simulation_bounds, self.border_width
        )
